...
public class UserJsonTest {
	...
	@Test
	public void deveVerificarComLista () {
		RestAssured.given()
		.when()
	...
	@Test
	public void devoFazerVerificoesAvancadas() {
		RestAssured.given()
		.when()
			.get("http://restapi.wcaquino.me/users")
		.then()
		.statusCode(200)
		.body("$", Matchers.hasSize(3))
		.body("age.findAll{it <= 25}.size()", Matchers.is(2)) // o IT verifica o age, ele pega a primeira referencia de parametro.
		.body("age.findAll{it <= 25 && it > 20}.size", Matchers.is(1))
		.body("findAll{it.age <= 25 && it.age > 20}.name", Matchers.hasItem("Maria Joaquina"))
		.body("findAll{it.age <= 25}[0].name", Matchers.is("Maria Joaquina")) // começa a verificar todos os itens e depois filtra por idade e [0] pega o primeiro item da lista.
		.body("findAll{it.age <= 25}[-1].name", Matchers.is("Ana Júlia")) // verifica o último item da lista.
		...
		.body("name.findAll{it.startsWith('Maria')}.collect{it.toUpperCase()}.toArray()", 
				Matchers.anyOf(Matchers.arrayContaining("MARIA JOAQUINA"), Matchers.arrayWithSize(1))) // verificar, precisa converter para string
